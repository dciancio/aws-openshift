...
data "template_file" "nodes_master" {
  ...
  vars = {
    ...
    oshost = var.ocp_version != "3.10" && var.ocp_version != "3.11" ? format(
      "%s=%s",
      "openshift_hostname",
      element(aws_instance.master.*.private_dns, count.index),
    ) : ""
...
data "template_file" "nodes_infra" {
  ...
  vars = {
    ...
    oshost = var.ocp_version != "3.10" && var.ocp_version != "3.11" ? format(
      "%s=%s",
      "openshift_hostname",
      element(aws_instance.infra.*.private_dns, count.index),
    ) : ""
...
data "template_file" "nodes_worker" {
  ...
  vars = {
    ...
    oshost = var.ocp_version != "3.10" && var.ocp_version != "3.11" ? format(
      "%s=%s",
      "openshift_hostname",
      element(aws_instance.worker.*.private_dns, count.index),
    ) : ""
